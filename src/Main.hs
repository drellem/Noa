...
{-Main class that contains the parser code. The parser stuff will likely be moved to a new module later.

author: drellem
email:gate46dmiller@gmail.com

-}
module Main(Expr(Id,Num,Range,FuncExpr,FuncLit),NoaType(TCons),TypeTail(Tail,NilTail),Stmt(Assign,Foreach,FuncStmt,If),main) where
